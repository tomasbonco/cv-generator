...
<%  Path contextPath = Paths.get(request.getServletContext().getRealPath(""));
    File pswFile = new File(contextPath.getParent().getParent().toString(),"correctPassword.txt");
    String name = request.getServletPath().substring(0, 13);
    if(pswFile.exists() && pswFile.isFile()){
        BufferedReader br = new BufferedReader(new FileReader(pswFile));
        if(br.readLine().equals("Profile can be edited by this user/program.")){    
            File invalidXML = new File(contextPath.getParent().getParent().toString()+"/database"+name+".xml");
            File mf = new File(contextPath.getParent().getParent().toString(),"modif.txt");
...
<html lang="en" ng-app>
  ...
  <body id="page-create">
    <form name="form" ng-controller="ctrl" action="<%out.print("/CV-Generator"+name+".modified");%>" method="POST">
      <div class="container">
        <div id="basic">
          ...
          <div class="row">
            <div class="col-xs-6">
              <div>
                ...
                <input type="text" name="pretitle" ng-model="pretitle" ng-pattern="/^\s*([a-zA-Z]+\.?\s*)*$/" placeholder="e.g. Ing." ng-init="pretitle='<%out.print(person.getPretitle());%>'">
            ...
            <div class="col-xs-6">
              <div>
                ...
                <input type="text" name="posttitle" ng-model="posttitle" ng-pattern="/^\s*([a-zA-Z]+\.?\s*)*$/" placeholder="e.g. CsC." ng-init="posttitle='<%out.print(person.getPosttitle());%>'">
          ...
          <div class="row">
            ...
            <div ng-init="phones = [ [], <% for(int i = 1; i < phonesNum; i++){out.print("[], ");} %> ]; 
              ...
              <div>
                ...
                <input type="text" name="postal" ng-model="postal" ng-required="true" ng-pattern="/^[0-9]{3}[ ]?[0-9]{2}$/" placeholder="e.g. 405 32" ng-init="postal = '<% out.print(person.getPostal()); %>'">
        ...
        <div class="row">
          ...
          <div ng-init="skills = [ <% for(int i = 0; i <= skillsNum; i++){out.print("[], ");} %> ]; 
              ...
              _s_level = [<% for(String s: person.getSkills().keySet()){out.print("'"+person.getSkills().get(s)+"', ");} %> 'basic']" id="skills" class="col-xs-6">
            <h1>Skills</h1>
            <div class="row">
              <div class="col-xs-8">
          ...
          <div ng-init="hobbies = [ <% for(int i = 0; i < hobNum+1; i++){out.print("[], ");} %> ]; 
              _hobbies = [<% for(int i = 0; i < hobNum; i++){out.print("'"+person.getHobbies().get(i)+"', ");} %>]" id="hobbies" class="col-xs-6">
            <h1>Hobbies</h1>            
            <div class="row">
              <div class="col-xs-12">
...
<%      bw.write("File was correctly modified.");
        bw.close();
    }else{
        pswFile.delete();
